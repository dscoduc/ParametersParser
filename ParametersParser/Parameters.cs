...
{
    /// <summary>
    /// Credit to https://www.siepman.nl/blog/ultimate-command-line-arguments-parsing-query-with-linq
    /// </summary>
    public class Parameters : IEnumerable<Parameter>
    {
        ...
        public Parameters(string parametersText = null, bool caseSensitive = false, char keyValuesplitter = '=')
        {
            _caseSensitive = caseSensitive;
            ParametersText = parametersText != null ? parametersText : GetAllParametersText();
            _parameters = new BareParameters(ParametersText, keyValuesplitter).Parameters.ToList();
        }

        public Parameters(bool caseSensitive) : this(null, caseSensitive)
        { }
        ...
        {
            get { return ParametersWithDistinctKeys.Select(p => p.Key); }
        }

        public IEnumerable<Parameter> ParametersWithDistinctKeys
        {
            get { return _parameters.GroupBy(k => k.Key, ThisEqualityComparer).Select(k => k.First()); }
        }
        ...
        {
            get { return _caseSensitive ? StringComparer.Ordinal : StringComparer.OrdinalIgnoreCase; }
        }

        private StringComparison ThisStringComparison
        {
            get { return _caseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase; }
        }

        public bool HasHelpKey
        {
            get { return HelpParameters.Any(h => _parameters.Any(p => p.Key.Equals(h, StringComparison.OrdinalIgnoreCase))); }
        }
